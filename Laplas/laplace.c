...
/*
    ...
    MPI_SEND(ptrsource,N-1,MPI_DOUBLE,destID,1,MPI_COMM_WORLD,status);
    destID=taskID-1;
    ptrsource=&TT[(taskID+1)*rowsinblock][1];//source
    MPI_SEND(ptrsource,N-1,MPI_DOUBLE,destID,2,MPI_COMM_WORLD,status);
        printf("iteration %5d task  %5d\n",itercount,taskID);
    }
...
//  MPI_SEND(ptrsource,N-1,MPI_DOUBLE,destID,1,MPI_COMM_WORLD,status);
        printf("after iteration %5d task  %5d\n",itercount,taskID);
...
//receive_____________________________________________
    ...
    MPI_IRECV(ptrdest,N-1,MPI_DOUBLE,sourceID,1,MPI_COMM_WORLD, status);
    sourceID=taskID-1;
    ptrdest=&TT[taskID*rowsinblock][1];
    MPI_IRECV(ptrdest,N-1,MPI_DOUBLE,sourceID,2,MPI_COMM_WORLD,status);
    }
    else if(taskID=0)
    {
    sourceID=taskID+1;
